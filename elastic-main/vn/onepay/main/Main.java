...
import vn.onepay.main.repositories.MOService;
import vn.onepay.mo.model.MO;
...
public class Main {
	...
	private void moIndexRepository() {
		...
		MOService myMOService = (MOService)ctx1.getBean("myMOService");
		...
		List<MO> mos = mongoTemplate.findAll(MO.class);
	    if ((mos != null) && (mos.size() > 0)) {
	  	...
	  	//delete index
	  	elasticsearchTemplate.deleteIndex(ESMO.class);
	  	
	    //INDEX
	    if(!elasticsearchTemplate.indexExists(ESMO.class)){
    	  System.out.println("Dang danh chi muc ...");
    	  
    	  List<String> provider = Arrays.asList(new String[]{"mwork", "ngan_luong", "homedirect", "vdconline"});
    	  
    	  List<ESMO> objList = new ArrayList<ESMO>();
    	  List<String> ids = new ArrayList<String>();
    	  for(MO mo : mos){
    		  ids.add(mo.getId());
    		  objList.add(new ESMO(mo.getId(), mo.getCp_code(), mo.getContent_id(), getRandomList(provider), mo.getTelco(), 
