...
public class MOAnalyticController extends AbstractProtectedController{
	...
	protected ModelAndView handleRequest(HttpServletRequest request,
		...
		int page = 0;
	      ...
	      if(elasticSearchService.checkIndex(ESMO.class)){
	    	    count = elasticSearchService.count(fields, terms, keywords, facetSize, ESMO.class);
	    	    termLists = elasticSearchService.getFacets(fields, terms, keywords, facetSize, ESMO.class);
	    	    termAllLists = elasticSearchService.getFacets(fields, null, null, facetSize, ESMO.class);
	    	    
	    	    dataList = elasticSearchService.search(fields, terms, keywords, sorts, 0/*page*/, count/*limit*/, facetSize, ESMO.class);
	      }
	    ...
	    if(tab.equalsIgnoreCase("") || tab.equalsIgnoreCase("subscriber") || !tab.equalsIgnoreCase("merchant")){
		    ...
		    if(topMsisdns != null && topMsisdns.size() > 0){
		    	for(FilteringMO data : topMsisdns){
		    		...
		    		dataHistogramMap.put(data.getMsisdn(), elasticSearchService.getHistogramFacet(fieldHistogram, fields, terms, keywords, facetSize, ESMO.class));
	    	...
	    	List<FilteringMO> topMerchant = new ArrayList<FilteringMO>();
		    ...
		    if(topMerchant != null && topMerchant.size() > 0){
		    	for(FilteringMO data : topMerchant){
		    		fieldFilter = "merchant";
		    		keywords.put(fieldFilter + operator, Arrays.asList(new String[] { data.getMerchant().toLowerCase()}));
		    		dataHistogramMap.put(data.getMerchant(), elasticSearchService.getHistogramFacet(fieldHistogram, fields, terms, keywords, facetSize, ESMO.class));
	...
	public static List<FilteringMO> getTopMsisdn(List<ESMO> sortedDataList, int limit) {
		if(sortedDataList == null || sortedDataList.size() == 0)
			return null;
		
		List<FilteringMO> topMsisdn = new ArrayList<FilteringMO>();
		for(ESMO mo : sortedDataList){
			//limit
			...
			for(int i = 0; i < topMsisdn.size(); i++){
				if(topMsisdn.get(i).getMerchant().equalsIgnoreCase(mo.getMerchant()) && topMsisdn.get(i).getMsisdn().equalsIgnoreCase(mo.getMsisdn())){
					index = i;
					break;
				}
			}
			...
			if(index != -1){
				...
				topMsisdn.add(new FilteringMO(mo.getMsisdn(), mo.getMerchant(), 1, 1, mo.getAmount()));
	...
	public static List<FilteringMO> getTopMerchant(List<ESMO> sortedDataList, int limit) {
		if(sortedDataList == null || sortedDataList.size() == 0)
			return null;
		...
		for(ESMO mo : sortedDataList){
			...
			if(index != -1){
				...
				topMerchant.add(new FilteringMO("", mo.getMerchant(), 1, 1, mo.getAmount()));
	...
	private List<ESMO> handleSortWithSubscriberBySum(List<ESMO> dataList){
	...
	@SuppressWarnings("unchecked")
	public static List<ESMO> handleSortWithSubscriberByCount(List<ESMO> dataList, int limit){
		if(dataList == null || dataList.size() == 0)
			return null;
	...
	@SuppressWarnings("unchecked")
	public static List<ESMO> handleSortWithMerchantByCount(List<ESMO> dataList, int limit){
		if(dataList == null || dataList.size() == 0)
			return null;
	...
	private List<ESMO> handleSortWithMerchantBySum(List<ESMO> dataList){
		if(dataList == null || dataList.size() == 0)
			return null;
