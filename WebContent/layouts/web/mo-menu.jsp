	...
	$(document).ready(function(){
		...
		$.each(keys, function(k , v){
			...
			$('.scope_search .'+v+'').live('click', function(){
				...
				var itemChild = $('form[name=listData] input[name='+v+']');
				
				if(typeof itemChild.val() === 'undefined'){
					 $('form[name=listData]').append('<input type="hidden" name="'+v+'" value="'+item+'"/>');
				}
				
				itemChild.val(item);
				
				$('form[name=listData]').submit();
			});
		...
		$('.scope_search .filter').live('click', function(){
			...
			$('form[name=listData] input[name='+child+']').remove();
